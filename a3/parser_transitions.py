...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            last_e = self.stack.pop()
            second_last_e = self.stack.pop()
            self.stack.append(last_e)
            self.dependencies.append((last_e, second_last_e))
        elif transition == "RA":
            last_e = self.stack.pop()
            second_last_e = self.stack[-1]
            self.dependencies.append((second_last_e, last_e))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        processing_parses = unfinished_parses[-batch_size:] 
        transitions = model.predict(processing_parses)
        for i, parse in enumerate(processing_parses):
            parse.parse_step(transitions[i])
            if len(parse.stack) == 1 and len(parse.buffer) == 0:
                dependencies[partial_parses.index(parse)] = parse.dependencies
