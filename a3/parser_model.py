...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        ### END YOUR CODE
        batch_size, n_feature = w.shape
        x = torch.zeros((batch_size, n_feature, self.embed_size))
        for i, sample in enumerate(w):
            for j, token in enumerate(sample):
                x[i][j] = self.embeddings.data[token]
    ...
    def forward(self, w):
        ...
        h = nn.functional.relu(torch.matmul(embeddings, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
