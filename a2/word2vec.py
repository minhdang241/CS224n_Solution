...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    score = softmax(outsideVectors@centerWordVec)
    loss = -np.log(score[outsideWordIdx])
    
    y = np.zeros(score.shape)
    y[outsideWordIdx] = 1
    gradCenterVec = outsideVectors.T@(score - y)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in range(outsideVectors.shape[0]):
        if i != outsideWordIdx:
            gradOutsideVecs[i] = centerWordVec*score[i]
        else:
            gradOutsideVecs[i] = centerWordVec*(score[outsideWordIdx] - 1) 
...
def negSamplingLossAndGradient(
    ...
    score = np.dot(outsideWordVec, centerWordVec)
    gradCenterVec = np.zeros(centerWordVec.shape)
    gradOutsideVecs = np.zeros(outsideVectors.shape)

    loss = -np.log(sigmoid(score))
    gradCenterVec += (sigmoid(score) - 1)*outsideWordVec
    gradOutsideVecs[outsideWordIdx] += (sigmoid(score) - 1)*centerWordVec

    for i in negSampleWordIndices:
        negWordVec = outsideVectors[i]
        score = np.dot(negWordVec, centerWordVec)
        loss += -np.log(sigmoid(-score)) 
        gradCenterVec += (1 - sigmoid(-score))*negWordVec
        gradOutsideVecs[i] += (1 - sigmoid(-score))*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        l, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
